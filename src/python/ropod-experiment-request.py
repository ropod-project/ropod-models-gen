...
#
#     result = ropod_experiment_request_from_dict(json.loads(json_string))
...
class GenericType(Enum):
    ...
    here.
    """
    EXPERIMENT_REQUEST = u"EXPERIMENT-REQUEST"
...
class Payload:
    ...
    def from_dict(obj):
        assert isinstance(obj, dict)
        experiment_type = from_str(obj.get(u"experimentType"))
        return Payload(experiment_type)

    def to_dict(self):
        result = {}
        result[u"experimentType"] = from_str(self.experiment_type)
        return result
...
class RopodExperimentRequest:
    ...
    def from_dict(obj):
        ...
        return RopodExperimentRequest(header, payload)
...
def ropod_experiment_request_from_dict(s):
    return RopodExperimentRequest.from_dict(s)


def ropod_experiment_request_to_dict(x):
    return to_class(RopodExperimentRequest, x)
