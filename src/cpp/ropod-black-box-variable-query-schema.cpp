...
namespace quicktype {
     ...
     * here.
     */
    enum class TimeStamp : int { VARIABLE_QUERY };
...
namespace nlohmann {
    ...
    inline void from_json(const json & j, quicktype::TimeStamp & x) {
        if (j == "VARIABLE-QUERY") x = quicktype::TimeStamp::VARIABLE_QUERY;
        else throw "Input JSON does not conform to schema";
    }

    inline void to_json(json & j, const quicktype::TimeStamp & x) {
        switch (x) {
            case quicktype::TimeStamp::VARIABLE_QUERY: j = "VARIABLE-QUERY"; break;
            default: throw "This should not happen";
        }
    }
}
